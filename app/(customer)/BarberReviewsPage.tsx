...
export default function BarberReviewsPage() {
  ...
  const employeeId = params.employeeId as string || 'employee-1';
  const barberName = params.barberName as string || 'Shark.l1';
  const barberPhoto = params.barberPhoto as string | undefined;
  ...
  const loadReviews = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const reviewsData = await customerAPI.getProfileReviews(employeeId, 'employee');
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (editingReviewId) {
        ...
        // Create new review
        const newReview = await customerAPI.addProfileReview(employeeId, 'employee', reviewRating, reviewText.trim());
  ...
  const performDelete = async (reviewId: string) => {
    ...
    try {
      ...
      const reloadedReviews = await customerAPI.getProfileReviews(employeeId, 'employee');
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.barberInfo}>
          {barberPhoto && (
            <Image source={{ uri: barberPhoto }} style={styles.barberImage} />
          )}
        ...
        <View style={styles.reviewsList} key={`reviews-${reviews.length}-${refreshKey}`}>
          {reviews.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyStateText}>Be the first to review this barber!</Text>
            </View>
          )}
        </View>
      </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.reviewModalContent}>
            ...
            <ScrollView style={styles.reviewModalBody}>
              <View style={styles.barberInfoModal}>
                {barberPhoto && (
                  <Image source={{ uri: barberPhoto }} style={styles.barberImageModal} />
                )}
                <Text style={styles.barberNameModal}>{barberName}</Text>
              </View>
